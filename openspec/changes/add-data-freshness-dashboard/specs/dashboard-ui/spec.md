...
- **THEN** 顯示該 BU 各群組卡片，含健康度百分比（`round((total-breached)/total*100)`）、群組名、
  以及「{n} 張逾期 · 共 {total} 張」或「全部正常 · 共 {total} 張」
...
#### Scenario: 開啟群組列出資料表
- **WHEN** 使用者於首頁點選某群組
- **THEN** 推入群組→資料表畫面，列出該群組各表卡（狀態點、表名、來源 chip、SLA、延遲、狀態 pill），且篩選為「全部」
...
### Requirement: 資料表詳情含真實 24h 趨勢
系統 SHALL 提供詳情頁，含狀態條、24h 延遲趨勢卡與資訊卡（最後更新時間、檢查時間、資料來源、
SLA 門檻、實際延遲）。趨勢圖 MUST 以真實歷史資料點繪製，並畫出 y=SLA 的水平虛線門檻與端點。

#### Scenario: 顯示詳情數值
- **WHEN** 使用者開啟某表詳情頁
- **THEN** 狀態條顯示 SLA 達標/逾期與延遲；資訊卡顯示最後更新時間、檢查時間、資料來源、SLA 門檻、實際延遲

#### Scenario: 趨勢圖用真實歷史
- **WHEN** 詳情頁取得該表近 24h 歷史資料點
- **THEN** 以折線 + 面積繪製真實延遲趨勢，並在 y=SLA 畫虛線門檻，x 軸標示 `−24h … 現在`

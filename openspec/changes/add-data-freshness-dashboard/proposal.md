...
（自有網域 `hsuping.org`）的儀表板，讓營運人員一眼看出哪些表逾期、整體健康度，並能下鑽到單表細節。
...
- 重建附件「Slate Ops」設計的四個畫面：首頁（依群組／依資料表 + BU 切換）、群組→資料表、
  資料表→各群組、資料表詳情（24h 趨勢）。導覽為 stack push/pop。
- 詳情頁的 24h 延遲趨勢圖**使用真實歷史**（即時抓該表近 24h 的 `Check_Time`/延遲資料點繪製）。
...
- `sheet-data-access`: 從 HK Google Sheet 經 gviz 讀取、剝除 JSONP 包裝、正規化成共同資料模型、
  選出最新快照，以及為詳情頁抓取單表近 24h 歷史。
- `dashboard-ui`: 四個畫面的版面與互動、stack 式導覽、BU／模式／篩選控制、群組健康度與狀態色彩、
  24h 趨勢 SVG，以及載入／錯誤／空資料狀態，遵循 Slate Ops 設計 token 與響應式規則。

# 人工驗收案例（每次改 UI 後重跑）

前置：`cd dashboard && python -m http.server 8080`，開 http://localhost:8080/
（手機外框：http://localhost:8080/MobilePreview.html）

## A. 密碼閘（access-gate）
- [ ] A1 首次開啟顯示密碼卡；輸入錯誤密碼 → 顯示「密碼錯誤」、不進入。
- [ ] A2 輸入 `53343286@Di` → 進入主畫面。
- [ ] A3 重新整理頁面 → 不再要求密碼（localStorage 記住）。
- [ ] A4 點右上「⎋」登出 → 重新要求密碼。

## B. 首頁（menu）
- [ ] B1 副標顯示「更新 {時間} · N 群組 / M 張表」。
- [ ] B2 預設「依群組」+ BU=MCD：顯示 cxgroup/segroup/mcwgroup 卡，每卡左側健康度色塊（紅=有逾時/綠=全正常）。
- [ ] B3 點 BU=BKW → 變成 bjgroup 卡（清單重新推導）。
- [ ] B4 切「依資料表」→ 顯示該 BU 不重複資料表列 + 狀態 pill。

## C. 下鑽與導覽（nav stack）
- [ ] C1 點一張群組卡 → 進入該群組資料表清單；左上「‹」可返回首頁。
- [ ] C2 群組頁點「只看異常」→ 只剩 Breached 的表；點「Realtime/Offline」對應篩選；切群組頁時篩選回「全部」。
- [ ] C3 「依資料表」點一張表 → 進入「該表在各群組」頁，只列含此表的群組。
- [ ] C4 任一表卡點進去 → 詳情頁。

## D. 詳情頁（近七天累積 Downtime）
- [ ] D1 狀態條標籤固定「近七天累積 Downtime」、右側顯示累積延遲（人性化），顏色依狀態（紅/綠）。
- [ ] D2 趨勢卡先顯示 spinner，稍後出現 SVG 折線 + SLA 虛線門檻 + 端點（真實七天歷史），X 軸為 4 個 `MM/DD HH時` 刻度。
- [ ] D3 資訊卡四列：檢查時間、資料更新時間、近七天 SLA 總時數、近七天累積 Downtime（末列顏色=狀態色）。
- [ ] D4 找一張歷史很少的表，趨勢卡顯示「尚無歷史資料點」或單點，不報錯。

- [ ] D5 詳情頁頂端顯示「概覽」／「歷史」兩個頁籤；預設「概覽」且維持既有近七天彙總、趨勢 SVG、資訊卡正常。
- [ ] D6 點「歷史」頁籤 → 出現範圍膠囊（近24小時／近3天／近7天）、摘要列「{範圍} · 共 N 筆 · M 筆逾時」、逐筆記錄表（檢查時間／資料更新時間／延遲時間／逾時狀態）；切換範圍即時重算、不重新抓取。逾時列延遲紅色、正常列綠色；逾時判定為 Delay > SLA。
- [ ] D7 詳情全域頁首副標在「{BU} · {group}」後顯示來源 chip（Realtime 藍色系／Offline 灰色系）與「SLA 15分」或「SLA 4小時」；返回後再開另一張表頁籤重設為「概覽」。
- [ ] D8 概覽趨勢卡在 X 軸刻度下方顯示三格統計磚：峰值（紅字）、平均（深灰字）、谷值（綠字），數值為 human 格式；若尚無資料點則磚不顯示。
- [ ] D9 切換「概覽」↔「歷史」頁籤時，內容區有 scrFade 淡入動畫（translateY 9px → 0，約 0.26s）；切換範圍膠囊亦有同樣動畫。
- [ ] D10 歷史記錄列滑鼠懸停時背景漸變為 #F7F9FB（transition 0.14s）；離開後恢復透明。
- [ ] D11 概覽趨勢圖 Hover-to-Inspect：滑鼠移入圖表 → 出現虛線十字準線 + 空心標記點，上方讀數更新為 MM/DD HH:mm + 延遲值 + 逾時/正常膠囊；移至不同點時讀數即時更新；移出圖表後讀數恢復「最新 MM/DD HH:mm」與最新點數值。
- [ ] D12 歷史頁籤排程空窗加註：當該範圍記錄中有整點缺漏（相鄰間隔 > 排程節奏×容忍係數）時，摘要列後出現琥珀色「⚠ K 個排程空窗（缺 M 筆）」；節奏正常時不顯示。

## D-anim. 卡片微互動（cards）
- [ ] DA1 首頁／群組／跨群組頁的卡片滑鼠懸停時上浮（translateY -2px）+ 陰影；按下時輕微縮小（scale .995）。
- [ ] DA2 逾時（Breached）狀態點持續脈動（dotPulse 1.8s）；正常狀態點不脈動。
- [ ] DA3 每次 git push 後，index.html 的 `?v=` 版號需 +1，確保瀏覽器載入最新 JS/CSS（否則會吃到快取舊檔）。

## H. 排程空窗警示（schedule-gap）
- [ ] H1 當最新快照 Check_Time 距現在超過 `staleThresholdMin`（預設 90 分）→ 全域頁首下方出現琥珀色橫幅「⚠ 排程可能中斷 · 資料已 {人性化} 未更新 · 最後更新 {時間}」；未過時則無橫幅。
- [ ] H2 橫幅在所有畫面（首頁/群組/詳情）皆隨頁首固定顯示。
- [ ] H3 （模擬）將 `staleThresholdMin` 暫設為 1 → 重新整理應出現橫幅；改回 90 後消失。確認 `?v=` 已 +1 避免快取。

## E. 狀態與重新整理
- [ ] E1 點「↻」→ 短暫載入後資料與副標時間更新。
- [ ] E2（模擬錯誤）把 config.js 的 HK id 暫改成錯的 → 重新整理頁面 → 顯示「讀取失敗…請確認 sheet 已設為可檢視」+「重試」鈕；改回後重試恢復。

## F. 響應式（手機/電腦）
- [ ] F1 桌面寬視窗：卡片多欄。
- [ ] F2 DevTools 切 iPhone 寬（≤390px）或開 MobilePreview.html：卡片單欄、不溢出、可點擊操作。

## G. 部署（deployment）
- [ ] G1 `git ls-files | grep -i '\.R$'` 回 CLEAN（無 .R 憑證入庫）。
- [ ] G2 `CNAME` 內容為 `hsuping.org`。
- [ ] G3 push 到 GitHub 公開 repo；Settings→Pages→Source=main/root。
- [ ] G4 DNS 設 A 記錄 185.199.108.153 / 109.153 / 110.153 / 111.153。
- [ ] G5 憑證簽發後勾 Enforce HTTPS；https://hsuping.org 可開、顯示儀表板。
- [ ] G6 手機實機開 https://hsuping.org 確認可查詢、可下鑽。

...
| 詳情頁 | **近七天累積 Downtime**：用真實歷史（即時抓該表近七天 Check_Time/Delay_Time 加總與繪製） |
...
瀏覽器 (GitHub Pages, hsuping.org)
  ...
  │  5. 渲染四個畫面（Slate Ops）
  │  6. 進詳情頁 → fetch gviz 該表近七天歷史 → 加總 Downtime/SLA + 畫真實趨勢
  ▼
（無後端；所有運算在前端）
```
...
**詳情頁真實歷史**（單表近七天）：
```
tq = select F, I where A = '{bu}' and B = '{group}' and C = '{table}' order by F desc limit 800
```
- 取回後反轉為時間順序，過濾近七天（`trendDays`）窗內的點；窗內無點則退回現有全部、皆無則空陣列。
- 由窗內點計算：累積 Downtime = Σ Delay_Time、近七天 SLA 總時數 = SLA × 點數。
- 以窗內 (時間, 延遲) 畫線，SLA 門檻虛線在 y = sla，X 軸標 4 個 `MM/DD HH時` 日期刻度（由 checkTime 推算）。
...
1. **首頁 Menu**：標題「資料觀測」+ 副標「更新 {checkTime} · N 群組 / M 張表」。
   ...
   內容：依群組 → 群組卡片（健康度色塊 + 名稱 + 「{n} 張逾時 · 共 {total} 張」）；
...
3. **資料表 → 各群組**：標題（表名 + BU badge）+ 各群組卡（群組名為標題，右側來源/SLA/延遲/pill）；
   只列該 BU 中含此表的群組。
4. **資料表詳情（近七天累積 Downtime）**：返回 + 標題（表名、`{BU} · {group}`）+ 狀態條（固定標籤
   「近七天累積 Downtime」+ 累積延遲人性化）+ **真實近七天趨勢卡**（SVG 折線 + 面積 + SLA 虛線 + 端點
   + 4 個 `MM/DD HH時` 日期刻度）+ 資訊卡 4 列（檢查時間/資料更新時間/近七天 SLA 總時數/近七天累積 Downtime）。

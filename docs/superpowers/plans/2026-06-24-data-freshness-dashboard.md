...
| 檔案 | 職責 | 狀態 |
|---|---|---|
| `core.js` | 純資料邏輯（無 DOM、無 fetch）：`parseGvizText`、`cellV`/`cellF`、`normalizeRow`、`selectLatestSnapshot`、`groupsInBU`、`distinctTables`、`buOf`、`parseTime`、`extractHistory`、`buildTrend`、`mkColors`、`filterRows`、`escHtml`、`sha256Hex` | 新增（從 `app.js` 抽出） |
...
- Consumes: `NormRow`
- Produces:
  - `DFCore.mkColors(row: NormRow): {accent, dotColor, delayColor, pillBg, pillText, pillLabel, srcBg, srcText}`。
...
匯出補上：`mkColors: mkColors, filterRows: filterRows, escHtml: escHtml`。
...
test('viewMenu byGroup shows health % and subtitle counts', () => {
  ...
  assert.match(html, /100%/);                // cxgroup: 2 列 0 逾期
  assert.match(html, /0%/);                  // segroup: 1 列 1 逾期
});
...
test('viewDetail renders info rows and trend when history ready', () => {
  const s = snap();
  const row = s.rows.find(function (r) { return r.group === 'segroup'; }); // Breached
  const html = Views.viewDetail({
    row: row, checkTime: s.checkTime,
    history: { status: 'ready', points: [{ delay: 10 }, { delay: 20 }] }
  });
  assert.match(html, /SLA 逾期/);
  assert.match(html, /最後更新時間/);
  assert.match(html, /<svg/);
  assert.match(html, /5時 0分/);
});
...
(function (root, factory) {
  ...
  function cardTable(row) {
    ...
    return '<button data-action="openDetail" data-id="' + row.id + '" style="display:flex;align-items:center;gap:12px;width:100%;text-align:left;background:#FFFFFF;border:1px solid #EAEDF2;border-left:3px solid ' + m.accent + ';border-radius:14px;padding:12px 13px;cursor:pointer;">' +
      ...
      '<div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:6px;">' +
        ...
        '<span style="font:500 10px \'JetBrains Mono\',monospace;color:#9AA3AF;">SLA ' + row.sla + '分</span></div>' +
      '</div>' +
    ...
    if (p.mode === 'byGroup') {
      var cards = C.groupsInBU(rows, p.bu).map(function (g) {
        ...
        return '<button data-action="openGroup" data-group="' + esc(g) + '" style="display:flex;align-items:center;gap:13px;width:100%;text-align:left;background:#FFFFFF;border:1px solid #EAEDF2;border-radius:16px;padding:15px;cursor:pointer;">' +
          ...
          '<div style="font:500 11.5px \'JetBrains Mono\',monospace;color:' + (bad ? '#C53D34' : '#1F8A5B') + ';margin-top:4px;">' + (bad ? (br + ' 張逾期') : '全部正常') + ' · 共 ' + t.length + ' 張</div></div>' +
      ...
      var tcards = C.distinctTables(rows, p.bu).map(function (tb) {
        ...
        return '<button data-action="openTable" data-table="' + esc(tb) + '" style="display:flex;align-items:center;gap:11px;width:100%;text-align:left;background:#FFFFFF;border:1px solid #EAEDF2;border-radius:14px;padding:13px 14px;cursor:pointer;">' +
          ...
          '<span style="font:600 10px \'Space Grotesk\',sans-serif;padding:3px 9px;border-radius:7px;background:' + (bad ? '#FCEAE7' : '#E6F4EC') + ';color:' + (bad ? '#C53D34' : '#1F8A5B') + ';flex:0 0 auto;">' + (bad ? (br + ' 逾期') : '正常') + '</span>' +
  ...
  function viewGroupTables(p) {
    ...
    var meta = br > 0 ? (all.length + ' 張 · ' + br + ' 逾期') : (all.length + ' 張 · 全部正常');
  ...
  function viewTableGroups(p) {
    ...
    var cards = rows.map(function (row) {
      ...
      return '<button data-action="openDetail" data-id="' + row.id + '" style="display:flex;align-items:center;gap:13px;width:100%;text-align:left;background:#FFFFFF;border:1px solid #EAEDF2;border-left:3px solid ' + m.accent + ';border-radius:14px;padding:14px;cursor:pointer;">' +
        ...
        '<span style="font:500 10px \'JetBrains Mono\',monospace;color:#9AA3AF;">SLA ' + row.sla + '分</span></div></div>' +
  ...
  function viewDetail(p) {
    ...
    var srcText = row.source === 'Realtime' ? 'Realtime（即時 15分）' : 'Offline（離線 240分）';
    var hist = p.history, trendInner;
    if (!hist || hist.status === 'loading') {
      ...
      var tr = C.buildTrend(hist.points, row.sla);
      trendInner = '<svg viewBox="0 0 300 84" preserveAspectRatio="none" style="width:100%;height:96px;display:block;overflow:visible;">' +
        ...
        '<div style="display:flex;justify-content:space-between;margin-top:8px;font:500 9.5px \'JetBrains Mono\',monospace;color:#B0B6BD;"><span>−24h</span><span>−18h</span><span>−12h</span><span>−6h</span><span>現在</span></div>';
    }
    ...
    return '<div style="max-width:860px;">' +
      ...
      '<div style="display:flex;align-items:center;justify-content:space-between;background:' + accentBg + ';border-radius:12px;padding:14px 16px;margin-top:16px;">' +
        ...
        '<div style="font:600 14px \'Space Grotesk\',sans-serif;color:' + accentText + ';">' + (br ? 'SLA 逾期' : 'SLA 達標') + '</div></div>' +
        '<div style="font:600 16px \'JetBrains Mono\',monospace;color:' + accentText + ';">' + esc(row.delayHuman) + '</div></div>' +
      '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px;margin-top:14px;">' +
        '<div style="background:#FFFFFF;border:1px solid #ECEEF1;border-radius:14px;padding:16px;">' +
          '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><div style="font:600 13px \'Space Grotesk\',sans-serif;color:#3A424C;">近 24 小時延遲趨勢</div>' +
          '<div style="display:flex;align-items:center;gap:5px;font:500 10px \'JetBrains Mono\',monospace;color:#9AA1AA;"><span style="display:inline-block;width:14px;height:0;border-top:2px dashed #99A0A8;"></span>SLA ' + row.sla + '分</div></div>' +
          trendInner +
        '</div>' +
        '<div style="background:#FFFFFF;border:1px solid #ECEEF1;border-radius:14px;overflow:hidden;align-self:start;">' +
          infoRow('最後更新時間', row.maxUpdate) +
          infoRow('檢查時間', p.checkTime) +
          infoRow('資料來源', srcText) +
          infoRow('SLA 門檻', row.sla + ' 分') +
          infoRow('實際延遲', row.delayMin + ' 分', accentText, true) +
        '</div>' +
      '</div></div>';
  }
...
test('snapshot pipeline: parse -> normalize -> latest -> menu render', () => {
  ...
  assert.match(html, /0%/); // bjgroup 1 列 1 逾期
});
...
在 `app.js` 內：
1. 刪除這些已搬到 `core.js` 的函式定義：`cellV`、`cellF`、`normalize`、`parseTime`、`rowsOfBU`、`groupsInBU`、`distinctTables`、`buOf`、`mk`、`esc`、`buildTrend`、`sha256Hex`。（`gvizUrl`、`gEsc` **保留**在 app.js — 它們屬於 fetch 層。）
...
```js
  function loadHistory(row) {
    var tq = "select F, I where A = '" + gEsc(row.bu) + "' and B = '" + gEsc(row.group) +
      "' and C = '" + gEsc(row.table) + "' order by F desc limit 200";
    return gvizQuery(row.region, tq).then(function (table) { return Core.extractHistory(table, CONFIG.trendHours); });
  }
```
...
- [ ] B2 預設「依群組」+ BU=MCD：顯示 cxgroup/segroup/mcwgroup 卡，每卡左側健康度色塊（紅=有逾期/綠=全正常）。
...
## D. 詳情頁（detail + 真實趨勢）
- [ ] D1 狀態條顯示「SLA 達標/逾期」與延遲，顏色正確（紅/綠）。
- [ ] D2 趨勢卡先顯示 spinner，稍後出現 SVG 折線 + SLA 虛線門檻 + 端點（真實歷史）。
- [ ] D3 資訊卡五列：最後更新時間、檢查時間、資料來源、SLA 門檻、實際延遲（延遲值顏色=狀態色）。
...
| dashboard-ui：詳情 + 真實趨勢 | Task 8 `viewDetail renders…` / `spinner while loading`；Task 10 D1–D4 |
